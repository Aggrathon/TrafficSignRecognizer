...
import os
import pygame
from config import DIR_FRAMES_SIGNS, DIR_CROPPED_SIGNS, DIR_FRAMES_NO_SIGNS, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGES_PER_SECOND, CROPPED_SIZE
from window import Window, get_rnd_filename, save_cropped


def no_sign(images, window, index, last):
    os.rename(os.path.join(DIR_FRAMES_SIGNS, images[index]), os.path.join(DIR_FRAMES_NO_SIGNS, images[index]))
    window.show_image(images[index + 1], DIR_FRAMES_SIGNS)
...
def undo(images, window, index, last):
    ...
    for i in range(index, last+1):
        try:
            os.rename(os.path.join(DIR_FRAMES_NO_SIGNS, images[i]), os.path.join(DIR_FRAMES_SIGNS, images[i]))
        except:
            pass
    window.show_image(images[index], DIR_FRAMES_SIGNS)
    return index, index
...
def on_mouse_click(images, window, index, last):
    ...
    img = pygame.image.load(os.path.join(DIR_FRAMES_SIGNS, images[index]))
...
def main():
    ...
    imgs = os.listdir(DIR_FRAMES_SIGNS)
    ...
    window.iterate(imgs, actions, DIR_FRAMES_SIGNS)
